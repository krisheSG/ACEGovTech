...
//                                 .assertThat().statusCode((Integer) statusCode);
                        response = RestAssured.given().contentType(ContentType.JSON).relaxedHTTPSValidation().headers(headersMap).get(url).then()
                                .assertThat().statusCode(statusCode);
                     //   response = RestAssured.given().relaxedHTTPSValidation().headers(headersMap).get(url).then().log().all();
